...
public function program_entries_save($quotation_id_arr,$attraction_arr, $program_arr, $stay_arr,$meal_plan_arr,$package_p_id_arr,$package_id_arr,$pckg_daywise_url)
{
    ...
    public function costing_entries_save($tcs_arr,$tcsvalue_arr,$quotation_id,$tour_cost_arr,$basic_amount_arr,$service_charge_arr,$service_tax_subtotal_arr,$total_tour_cost_arr,$transport_cost_arr,$excursion_cost_arr,$adult_cost_arr,$infant_cost_arr,$child_with_arr,$child_without_arr,$bsmValues,$package_type_c_arr,$discount_in_arr,$discount_arr)
    {
    	for($i=0; $i<sizeof($package_type_c_arr); $i++)
    	{
    		...
    		$sq_plane = mysqlQuery("insert into package_tour_quotation_costing_entries ( id, quotation_id, tour_cost,excursion_cost,basic_amount, service_charge, service_tax_subtotal, total_tour_cost, package_id, transport_cost,adult_cost,infant_cost,child_with,child_without,bsmValues,package_type,discount_in,discount) values ( '$id', '$quotation_id', '$tour_cost_arr[$i]','$excursion_cost_arr[$i]', '$basic_amount_arr[$i]', ' $service_charge_arr[$i]', '$service_tax_subtotal_arr[$i]', '$total_tour_cost_arr[$i]','0','$transport_cost_arr[$i]','$adult_cost_arr[$i]','$infant_cost_arr[$i]','$child_with_arr[$i]','$child_without_arr[$i]','$bsmvaluesEach','$package_type_c_arr[$i]','$discount_in_arr[$i]','$discount_arr[$i]')");
    		if(!$sq_plane){
