...
$last_name = addslashes($last_name);
  ...
  $username_count = mysqli_num_rows(mysqlQuery(" select * from roles where user_name= '$username'"));
  ...
  $emp_name_count = mysqli_num_rows(mysqlQuery("select * from emp_master where first_name='$first_name' and last_name='$last_name' and mobile_no='$mobile_no' and email_id = '$email_id'")); 
  ...
  if($emp_name_count>0){
    ...
    exit;
 	...
 	$row=mysqlQuery("select max(emp_id) as max from emp_master");
 	...
 	if($sq){
      ...
      $row1=mysqlQuery("select max(id) as max from roles");
      $value1=mysqli_fetch_assoc($row1);
      $max1=$value1['max']+1;

      $sq1 = mysqlQuery("insert into roles (id, role_id,branch_admin_id, emp_id, user_name, password, active_flag) values ('$max1', '$role_id', '$branch_id', '$max', '$username', '$password', '$active_flag')");
      ...
      if(!$sq1){
        ...
        //Email employee sign up notification
          $sq_cms_count = mysqli_num_rows(mysqlQuery("select * from cms_master_entries where id='1' and active_flag='Active'"));
          if($sq_cms_count != '0'){
 ...
 {  
   ...
   $sq = mysqlQuery("update emp_master set photo_upload_url='$photo_upload_url' where emp_id='$emp_id' ");
  if($sq){
 ...
 {  
  ...
  $first_name = addslashes($first_name);
  $last_name = addslashes($last_name);
  $cur_username_sq = mysqli_fetch_assoc(mysqlQuery("select * from roles where emp_id='$emp_id'"));
  ...
  $emp_name_count = mysqli_num_rows(mysqlQuery("select * from emp_master where first_name='$first_name' and last_name='$last_name' and mobile_no='$mobile_no' and email_id = '$email_id' and emp_id!='$emp_id'")); 
