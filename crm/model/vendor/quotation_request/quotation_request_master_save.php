...
public function quotation_request_save()
{
	...
	$cab_type = addslashes($_POST['cab_type']);
	$transfer_type = addslashes($_POST['transfer_type']);
	$enquiry_specification = addslashes($_POST['enquiry_specification']);
	$branch_admin_id = $_POST['branch_admin_id'];
	$emp_id = $_POST['emp_id'];

	$dynamic_fields = $_POST['dynamic_fields'];
	$dynamic_tbl = isset($_POST['dynamic_tbl']) ? $_POST['dynamic_tbl'] : [];
	...
	$sq_request = mysqlQuery("insert into vendor_request_master(request_id, enquiry_id, emp_id, quotation_for,city_id,vendor_city_id, branch_admin_id, tour_type, quotation_date, airport_pickup, cab_type, transfer_type, enquiry_specification, dynamic_fields, hotel_entries, dmc_entries, transport_entries, excursion_specification, created_at) values ('$request_id', '$enquiry_id', '$emp_id', '$quotation_for','$city_name','$city_id', '$branch_admin_id', '$tour_type', '$quotation_date', '$airport_pickup', '$cab_type', '$transfer_type', '$enquiry_specification', '$dynamic_fields', '$hotel_entries', '$dmc_entries', '$transport_entries', '$excursion_specification', '$created_at')");
	if($sq_request){
		if($GLOBALS['flag']){
			commit_t();
...
public function email_send($request_id,$enquiry_id)
{
	...
	$sq_request = mysqli_fetch_assoc(mysqlQuery("select * from vendor_request_master where request_id='$request_id'"));

	$quotation_for = $sq_request['quotation_for'];
	$date = $sq_request['quotation_date'];
    $yr = explode("-", $date);
    $year = $yr[0];
	...
	if($quotation_for=="Hotel"){
		...
		$sq_hotel = mysqlQuery("select * from hotel_master where hotel_id in($sq_request[city_id])");

		while($row_dmc = mysqli_fetch_assoc($sq_hotel)){
	...
	if($quotation_for=="DMC"){
		...
		$contact_person_arr = array();
		$supplier_id_arr = array();
		$sq_dmc = mysqlQuery("select * from dmc_master where dmc_id in($sq_request[city_id])");
	...
	if($quotation_for=="Transport"){
		...
		$sq_dmc1 = mysqlQuery("select * from transport_agency_master where transport_agency_id in($sq_request[city_id])");
		while($row_dmc1 = mysqli_fetch_assoc($sq_dmc1)){
...
public function quotation_mail($sq_request, $quotation_for, $to, $vendor_name, $vendor_address, $contact_person_name, $supplier_id_arr, $link, $request_id,$enquiry_id,$year)
{
	...
	$sq_request1 = mysqli_fetch_assoc(mysqlQuery("select * from vendor_request_master where request_id='$request_id'"));
	$quotation_for = $sq_request1['quotation_for'];
	$booker_id=$sq_request1['emp_id']; 
	$row=mysqli_fetch_assoc(mysqlQuery("select emp_id,first_name, last_name from emp_master where emp_id='$booker_id'"));
	...
	$count = 0;
	$enqDetails = mysqli_fetch_assoc(mysqlQuery("SELECT enquiry_content from enquiry_master where enquiry_id = '$enquiry_id'"));
	...
	foreach($dynamic_fields as $val){
		switch($val->name){
			...
			case "with_bed" : {$with_bed = $val->value;} break;
			case "without_bed" : {$without_bed = $val->value;} break;
	...
	if($quotation_for == "DMC"){
		...
		<tr>
			<table width="85%" cellspacing="0" cellpadding="5" style="color: #888888;border: 1px solid #888888;margin: 0px auto;margin-top:20px; min-width: 100%;" role="presentation">
				...
				<tr><td style="text-align:left;border: 1px solid #888888;">Activity Details</td>   <td style="text-align:left;border: 1px solid #888888;">'.$sq_request1['excursion_specification'].'</td></tr>
				<tr><td style="text-align:left;border: 1px solid #888888;">Passenger</td>   <td style="text-align:left;border: 1px solid #888888;">'.intval($t_adult).' Adult(s),'.(intval($without_bed)+intval($with_bed)).' Child(ren),'.intval($t_infant).' Infant(s)</td></tr>
			</table>
		</tr>
		';
		for($j=0; $j<sizeof($hotel_type);$j++){
			...
			<table width="85%" cellspacing="0" cellpadding="5" style="color: #888888;border: 1px solid #888888;margin: 0px auto;margin-top:20px; min-width: 100%;" role="presentation">
				...
				<tr><td style="text-align:left;border: 1px solid #888888;width:50%">Hotel Category</td>   <td style="text-align:left;border: 1px solid #888888;">'.$hotel_type[$j].'</td></tr>
		...
		<tr>
			<table width="85%" cellspacing="0" cellpadding="5" style="color: #888888;border: 1px solid #888888;margin: 0px auto;margin-top:20px; min-width: 100%;" role="presentation">
				...
				<tr><td style="text-align:left;border: 1px solid #888888;">Cab Type</td>   <td style="text-align:left;border: 1px solid #888888;">'.$sq_request1['cab_type'].'</td></tr>
				<tr><td style="text-align:left;border: 1px solid #888888;">Enquiry Specification</td>   <td style="text-align:left;border: 1px solid #888888;">'.$sq_request1['enquiry_specification'].'</td></tr>
				<tr><td style="text-align:left;border: 1px solid #888888;">Passenger</td>   <td style="text-align:left;border: 1px solid #888888;">'.$t_adult.' Adult(s),'.($without_bed+$with_bed).' Child(ren),'.$t_infant.' Infant(s)</td></tr>
			</table>
		</tr>
		';
	}
