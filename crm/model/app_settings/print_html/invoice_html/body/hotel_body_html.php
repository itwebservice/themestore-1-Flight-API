...
$tax_show = '';
$newBasic = 0;
$total_paid = floatval($total_paid) + floatval($credit_card_charges);
...
if($bg != ''){
  ...
  $balance = floatval($net_amount1) - floatval($total_paid) + floatval($charge);
}
      ...
      <div class="col-md-12">
        ...
        <table class="table table-bordered no-marg" id="tbl_emp_list" style="padding: 0 !important;">
          ...
          while($row_passenger = mysqli_fetch_assoc($sq_hotel_entry))
          {
              ...
              $sq_hotels = mysqli_fetch_assoc(mysqlQuery("select hotel_id, hotel_name from hotel_master where hotel_id='$row_passenger[hotel_id]'"));
            ?>
            <tr class="odd">
              <td><?php echo $count; ?></td>
              <td><?php echo $sq_city['city_name']; ?></td>
              <td><?php echo $sq_hotels['hotel_name']; ?></td>
              ...
              <td><?php echo (isset($sq_passengers['adults']) ? $sq_passengers['adults'] : 0).' Adult(s), '.(isset($sq_passengers['childrens']) ? $sq_passengers['childrens'] : 0).' Child(wo), '.(isset($sq_passengers['child_with_bed']) ? $sq_passengers['child_with_bed'] : 0).' Child(wi), '.(isset($sq_passengers['infants']) ? $sq_passengers['infants'] : 0).' Infant(s)'  ?></td>
            </tr>
            <?php   
                $count++;
              } ?>
...
<div class="row">
  <div class="col-md-12">
    <div class="main_block inv_rece_calculation border_block">
      <?php
        if($service_charge_switch == 'No'){ ?>
        <div class="col-md-6"><p class="border_lt"><span class="font_5">BASIC AMOUNT </span><span class="float_r"><?= $basic_service_amt1 ?></span></p></div>
      <?php }else{
