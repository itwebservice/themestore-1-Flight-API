...
while($row_booking = mysqli_fetch_assoc($sq_package))
{
	...
	if($sq_est_count!='0'){
		...
		$sq_tour_refund_count = mysqli_num_rows(mysqlQuery("select * from refund_traveler_estimate where tourwise_traveler_id='$row_booking[id]'"));
		...
		$cancel_amount = ($sq_tour_refund_count > 0) ? $sq_tour_refund['cancel_amount'] : 0;
	}
	//total amount
	...
	// Net amount
	$net_amount = 0;
	$net_amount  =  $row_booking['net_total'] + $row_booking['total_travel_expense'] - $cancel_amount;
	...
	$tour_subtotal = $row_booking['tour_fee_subtotal_1'] - $cancel_amount;
...
number_format(floatval($total_amount),2).$currency_amount,
number_format(floatval($sq_paid_amount['sum']),2),
