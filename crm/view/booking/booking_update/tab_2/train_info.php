<?php 
$sq_train_details_count = mysqli_num_rows(mysqlQuery("select * from train_master where tourwise_traveler_id='$tourwise_id'"));
...
<tr>
	...
	<td><input type="text" id="<?php echo 'txt_train_date'.$count_t.'t' ?>" name="<?php echo 'txt_train_date'.$count_t.'t' ?>" placeholder="Departure Date & Time" title="Departure Date & Time" value="<?php echo date("d-m-Y H:i", strtotime($row_train_details['date'])) ?>" style="width:156px"/></td>

	<td><select id="txt_train_from_location<?= $count_t ?>t" title="From Location" onchange="validate_location('txt_train_from_location<?= $count_t ?>t' , 'txt_train_to_location<?= $count_t ?>t')" name="txt_train_from_location<?= $count_t ?>t" class="app_select2 train_from" style="width:120px">
	    <option value="<?php echo $row_train_details['from_location'] ?>"><?php echo $row_train_details['from_location'] ?></option>  
	    ?>
	</select></td>
	<td><select id="txt_train_to_location<?= $count_t ?>t" title="To Location" onchange="validate_location( 'txt_train_to_location<?= $count_t ?>t' , 'txt_train_from_location<?= $count_t ?>t')" name="txt_train_to_location<?= $count_t ?>t" class="app_select2 train_to" style="width:120px">
	    <option value="<?php echo $row_train_details['to_location'] ?>"><?php echo $row_train_details['to_location'] ?></option>
	</select></td>

	<td><input type="text" id="<?php echo 'txt_train_no'.$count_t.'t' ?>" name="<?php echo 'txt_train_no'.$count_t.'t' ?>" placeholder="Train Name & No" title="Train Name & No" value="<?php echo $row_train_details['train_no'] ?>" style="width:100px"/></td>

	<td><input type="text" id="<?php echo 'txt_train_total_seat'.$count_t.'t' ?>" name="<?php echo 'txt_train_total_seat'.$count_t.'t' ?>" placeholder="Total Seats" title="Total Seats" maxlength="3" onchange="validate_balance(this.id);" value="<?php echo $row_train_details['seats'] ?>" style="width:100px"/></td>
...
<script>
	function generating_train_date()
    {
        ...
        {
            $( "#txt_train_date"+i+'t').datetimepicker({ format: "d-m-Y H:i"  });
        }             
    }
