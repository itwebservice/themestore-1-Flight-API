<?php 
$sq_cruise_details_count = mysqli_num_rows(mysqlQuery("select * from group_cruise_master where booking_id='$tourwise_id'"));
...
<?php
$count_t = 0;
$sq_cruise_details = mysqlQuery("select * from group_cruise_master where booking_id='$tourwise_id'");
...
<tr>
    ...
    <td><input type="text" id="cruise_arrival_date<?= $count_t ?>t" name="cruise_arrival_date<?= $count_t ?>t" placeholder="Arrival Date & Time" title="Arrival Date & Time" class="app_datetimepicker" value="<?= get_datetime_user($row_cruise_details['arrival_datetime']) ?>" style="width:156px"></td>
...
<script>
	$('#cruise_arrival_date<?= $count_t ?>t, #cruise_departure_date<?= $count_t ?>t').datetimepicker({ format : 'd-m-Y H:i'});
</script>
<?php   
}    
?>
