...
$cancel_amount = ($sq_c_info > 0) ? $sq_refund_estimate['cancel_amount'] : 0;
$refund_amount = ($sq_c_info > 0) ? $sq_refund_estimate['total_refund_amount'] : 0;

$sq_total_ref_count = mysqli_num_rows(mysqlQuery("select sum(total_refund) as sum from package_refund_traveler_cancelation where booking_id='$booking_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
$sq_total_ref_paid_amount = mysqli_fetch_assoc(mysqlQuery("select sum(total_refund) as sum from package_refund_traveler_cancelation where booking_id='$booking_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
$total_ref_paid = ($sq_total_ref_count > 0) ? $sq_total_ref_paid_amount['sum'] : 0;

$unique_timestamp =  md5(uniqid());
$sale_Amount = isset($sq_booking['net_total']) ? $sq_booking['net_total'] : 0;
$paid_amount =  isset($sq_total_tour_paid_amount['sum']) ? $sq_total_tour_paid_amount['sum'] : 0;

$remaining = floatval($refund_amount) - floatval($total_ref_paid);
?>
                ...
                <div class="stat_content main_block">
                    <span class="main_block content_span" data-original-title="" title="">
                        ...
                        <span class="stat_content-amount pull-right" data-original-title=""
                            title=""><?= get_package_booking_id($booking_id, $year) ?></span>
                    </span>
                    <span class="main_block content_span" data-original-title="" title="">
                    ...
                    <span class="main_block content_span" data-original-title="" title="">
                        ...
                        <span class="stat_content-amount pull-right" data-original-title="" title="">
                            <?= ($paid_amount == '') ? '0.00' : number_format($paid_amount, 2) ?></span>
                    </span>
                    <span class="main_block content_span" data-original-title="" title="">
    ...
    <div class="col-md-12 col-sm-12 col-xs-12 mg_tp_20">
        <form id="frm_traveler_refund">
            ...
            <div class="panel panel-default panel-body mg_bt_10">
                <div class="row">
                    <div class="col-sm-6 col-xs-12 mg_bt_10">
                        <input type="text" id="txt_total_refund_cost_c" name="txt_total_refund_cost_c"
                            placeholder="*Refund Amount" title="Refund Amount"
                ...
                <div class="row">
                    <div class="col-sm-6 col-xs-12 mg_bt_10">
                        <select id="cmb_refund_mode" name="cmb_refund_mode" title="Payment Mode" class="form-control"
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6 col-xs-12 mg_bt_10">
                        <select name="bank_id" id="bank_id" title="Bank" class="form-control" style="width:100%"
                            disabled>
                ...
                <div class="row col-xs-12 text-center mg_tp_20">
                    <button class="btn btn-sm btn-success" id="refund_btn"><i
...
$(function() {
    $('#frm_traveler_refund').validate({
        rules: {
            txt_unique_timestamp: {
                required: true
            },
            booking_id: {
                required: true
            },
            txt_total_refund_cost_c: {
                required: true,
                number: true
            },
            cmb_refund_mode: {
        ...
        submitHandler: function(form) {
            ...
            if (parseFloat(remaining) == 0 && parseFloat(refund_amount) > 0) {
            ...
            $.post(base_url + 'controller/package_tour/cancel_and_refund/refund_booking_c.php', {
                ...
                bank_name: bank_name,
                bank_id: bank_id,
                'traveler_id_arr[]': traveler_id_arr
            }, function(data) {
                msg_alert(data);
                $('#refund_btn').prop('disabled', false);
                reset_form('frm_traveler_refund');
                refund_booking_reflect();
