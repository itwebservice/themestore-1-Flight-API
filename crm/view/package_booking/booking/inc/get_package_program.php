      ...
      <div class="row">
          ...
          <table style="width:100%" id="package_program_list" name="package_program_list" class="table mg_bt_0 table-bordered">
              ...
              <?php while($row_tours = mysqli_fetch_assoc($sq_tours)){?>
                <tr>
                <td width="27px;" style="padding-right: 10px !important;"><input class="css-checkbox mg_bt_10 labelauty" id="chk_program<?= $count?>" type="checkbox" checked style="display: none;"><label for="chk_program1"><span class="labelauty-unchecked-image"></span><span class="labelauty-checked-image"></span></label></td>
                ...
                <td class='col-md-1 pad_8'><button type="button" class="btn btn-info btn-iti btn-sm" id="itinerary<?php echo $count; ?>" title="Add Itinerary" onClick="add_itinerary('dest_name2','special_attaraction<?= $count?>-u','day_program<?= $count?>-u','overnight_stay<?= $count?>-u','Day-<?=$count?>')"><i class="fa fa-plus"></i></button>
                </td>
                </tr>
              <?php $count++; } ?>
              </tbody>
