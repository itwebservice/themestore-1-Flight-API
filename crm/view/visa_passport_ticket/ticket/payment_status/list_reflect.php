...
while($row_ticket = mysqli_fetch_assoc($sq_ticket)){
	...
	if($row_ticket['cancel_type'] == '1'){
		...
		$bal = (($total_sale - floatval($cancel_estimate)) + $cancel_amount) - $paid_amount;
	}
	else{
	...
	$sq_purchase1 = mysqli_fetch_assoc(mysqlQuery("select * from vendor_estimate where status!='Cancel' and estimate_type='Flight' and estimate_type_id='$row_ticket[ticket_id]' and delete_status='0'"));	
	...
	$service_tax1 = $row_ticket['service_tax_subtotal'];
	//**Basic Cost
	$basic_cost = $row_ticket['basic_cost'] - $row_ticket['cancel_amount'];
	...
	$url1 = BASE_URL."model/app_settings/print_html/invoice_html/body/tax_invoice_html.php?invoice_no=$invoice_no&invoice_date=$invoice_date&customer_id=$customer_id&service_name=$service_name&basic_cost=$basic_cost&service_charge=$service_charge&taxation_type=&service_tax_per=&service_tax=$service_tax1&net_amount=$net_amount&ticket_id=$ticket_id&total_paid=$paid_amount&balance_amount=$bal_amount&sac_code=$sac_code&branch_status=$branch_status&pass_count=$pass_counts&credit_card_charges=$credit_card_charges";
	else
	$url1 = BASE_URL."model/app_settings/print_html/invoice_html/body/flight_body_html.php?invoice_no=$invoice_no&invoice_date=$invoice_date&customer_id=$customer_id&service_name=$service_name&basic_cost=$basic_cost&taxation_type=&service_charge=$service_charge&service_tax_per=&service_tax=$service_tax1&net_amount=$net_amount&ticket_id=$ticket_id&total_paid=$paid_amount&balance_amount=$bal_amount&sac_code=$sac_code&branch_status=$branch_status&credit_card_charges=$credit_card_charges";
	...
	$sq_incentive_count = mysqli_num_rows(mysqlQuery("select * from booker_sales_incentive where booking_id='$row_ticket[ticket_id]' and service_type='Ticket Booking'"));
	...
	$cancel_estimate_data = json_decode($row_ticket['cancel_estimate']);
	$cancel_estimate = (!isset($cancel_estimate_data)) ? 0 : $cancel_estimate_data[0]->ticket_total_cost;
	$sales_return = ($row_ticket['cancel_type'] == 1 || $row_ticket['cancel_type'] == 2 || $row_ticket['cancel_type'] == 3) ? number_format(floatval($cancel_estimate),2) : 'NA';
	...
	'<button class="btn btn-info btn-sm" onclick="ticket_view_modal('. $row_ticket['ticket_id'] .')" id="packagev_btn-'. $row_ticket['ticket_id'] .'" data-toggle="tooltip" title="View Details"><i class="fa fa-eye" aria-hidden="true"></i></button>',
	...
	number_format($row_ticket['ticket_total_cost'],2),
	number_format($cancel_amount, 2),
	number_format($total_bal, 2),
	number_format($paid_amount, 2),
	'<button class="btn btn-info btn-sm" id="paymentv_btn-'. $row_ticket['ticket_id'] .'" onclick="payment_view_modal('.$row_ticket['ticket_id'] .')"  data-toggle="tooltip" title="View Details"><i class="fa fa-eye" aria-hidden="true"></i></button>',
	$sales_return,
	number_format($bal, 2),
	$due_date,
	number_format($total_purchase,2),
	'<button class="btn btn-info btn-sm" id="supplierv_btn-'. $row_ticket['ticket_id'] .'" onclick="supplier_view_modal('. $row_ticket['ticket_id'] .')" data-toggle="tooltip" title="View Details"><i class="fa fa-eye" aria-hidden="true"></i></button>',
	$branch_name,
	$emp_name,
