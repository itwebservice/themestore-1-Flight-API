              ...
              <div class="table-responsive">
                  <table  class="table table-bordered no-marg">
                    ...
                    <tbody>
                      <?php 
                      $count = 0;
                      $sq_query = mysqlQuery("SELECT * FROM vendor_estimate WHERE status!='Cancel' and estimate_type = 'B2C' and estimate_type_id = '$booking_id' and delete_status='0'");
                      ...
                      while($row_entry = mysqli_fetch_assoc($sq_query))
                      {
                        ...
                        if($row_entry['purchase_return'] == 0){
                          ...
                          $cancel_estimate = (json_decode($row_entry['cancel_estimate'])[0] === null ) ? 0 : json_decode($row_entry['cancel_estimate'])[0]->net_total;
                          $p_purchase = ($row_entry['net_total'] - floatval($cancel_estimate));
