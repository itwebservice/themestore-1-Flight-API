...
<form id="frm_cred_info">
    <div class="row mg_tp_20 mg_bt_20 text-right">
        <div class="col-xs-12">
            <button type="button" class="btn btn-sm btn-success ico_left" data-toggle="modal"
                data-target="#c_view_modal" id="btn_view">&nbsp;&nbsp;View Format<i class="fa fa-eye"></i></button>
        </div>
    </div>
        ...
        <div class="row mg_tp_20 mg_bt_20">
            ...
            <div class="col-md-4">
                <select id="app_smtp_status" name="app_smtp_status" title="SMTP Status">
                    ...
                    <option value="<?= $sq_settings['app_smtp_status'] ?>"><?= $sq_settings['app_smtp_status'] ?>
                    </option>
                    <?php } ?>
                    <option value="No">No</option>
                    <option value="Yes">Yes</option>
                </select>
                <small>Note : Please select whether do you want smtp mailing</small>
            </div>
            ...
            <div class="col-md-4">
                <input type="text" id="app_smtp_host" onchange="fname_validate(this.id)" name="app_smtp_host"
                    placeholder="SMTP Host" title="SMTP Host" value="<?= $sq_settings['app_smtp_host'] ?>">
                <small>Note : Please enter smtp host name Ex. mail.itourscloud.com</small>
            </div>
        </div>
        ...
        <div class="row mg_bt_20">
            ...
            <div class="col-md-4">
                <input type="text" onchange="validate_alphanumeric(this.id);" id="app_smtp_port" name="app_smtp_port"
                    placeholder="SMTP Port" title="SMTP Port" value="<?= $sq_settings['app_smtp_port'] ?>">
                <small>Note : Please enter smtp port name Ex. 587</small>
            </div>
            ...
            <div class="col-md-4">
                <input type="password" id="app_smtp_password" name="app_smtp_password" class="form-control"
                    placeholder="SMTP Password" title="SMTP Password" value="<?= $sq_settings['app_smtp_password'] ?>">
                <small>Note : Please enter smtp password</small>
            </div>
        </div>
        ...
        <div class="row mg_bt_20">
            ...
            <div class="col-md-4">
                <input type="text" id="app_smtp_method" onchange="fname_validate(this.id);" name="app_smtp_method"
                    placeholder="SMTP Method" title="SMTP Method" value="<?= $sq_settings['app_smtp_method'] ?>">
                <small>Note : Please enter smtp method name Ex. SSL</small>
            </div>
    ...
    <div class="panel panel-default panel-body app_panel_style feildset-panel mg_bt_30">
        ...
        <div class="row mg_tp_20 mg_bt_20">
            ...
            <div class="col-md-4">
                <input type="text" id="sms_username" name="sms_username" placeholder="SMS API Username"
                    title="SMS API Username" value="<?= $sq_settings['sms_username'] ?>">
            </div>
            <div class="col-md-2 text-right"><label for="sms_password">SMS API Password</label></div>
            <div class="col-xs-8 col-md-3">
                <input type="password" id="sms_password" name="sms_password" class="form-control"
                    placeholder="SMS API Password" title="SMS API Password" value="<?= $sq_settings['sms_password'] ?>">
            </div>
            <div class="col-xs-4 col-md-1">
                <button type="button" class="btn btn-excel" onclick="show_password('sms_password')" title="View Password"><i class="fa fa-eye"></i></button>
            </div>
        </div>
    </div>

    <div class="panel panel-default panel-body app_panel_style feildset-panel mg_bt_30 hidden">
        <legend>Change Password</legend>
        <div class="row mg_tp_20 mg_bt_20">
            <div class="col-md-2 text-right"><label for="current_password">Current Password</label></div>
            <?php
            $sq_password = mysqli_fetch_assoc(mysqlQuery("select * from roles where emp_id='0'"));
            $password = $encrypt_decrypt->fnDecrypt($sq_password['password'], $secret_key); ?>
            <div class="col-md-4">
                <input type="password" id="current_password" name="current_password"
                    onchange="validate_password(this.id)" class="form-control" placeholder="Current password"
                    title="Current password" value="<?= $password ?>">
            </div>
            <a onclick="show_password('current_password')" target="_BLANK" class="btn btn-sm app_btn"
                title="View Password"><i class="fa fa-eye"></i></a>
            <a id="forgot_password"
                href="<?= BASE_URL ?>model/app_settings/send_admin_password.php?app_email_id=<?= $sq_settings['app_email_id'] ?>"
                onclick="return yes_js_login('<?= $sq_settings['app_email_id'] ?>');"><u>Forgot Password?</u></a>

        </div>
        <div class="row mg_bt_20">
            <div class="col-md-2 text-right"><label for="new_password">New password</label></div>
            <div class="col-md-4">
                <input type="password" class='form-control' id="new_password" onchange="validate_password(this.id)"
            ...
            <div class="col-md-4">
                <input type="password" class='form-control' id="re_password" onchange="validate_password(this.id)"
    ...
    <div class="panel panel-default panel-body app_panel_style feildset-panel mg_bt_30">
        ...
        <div class="row mg_tp_20 mg_bt_20">
            ...
            <div class="col-xs-8 col-md-5">
                <input name="ip_address" class="form-control" onchange="ValidateIPaddress(this.id,true);"
    ...
    <div class="row text-center mg_tp_20">
        <div class="col-md-12">
            <button class="btn btn-info btn-sm ico_left" type="button" onclick="run_reminders()"><i
                    class="fa fa-floppy-o"></i>&nbsp;&nbsp;Run Reminders</button>&nbsp;&nbsp;
    ...
    <div class="modal-dialog modal-lg" role="document">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body profile_box_padding">
                <?php
                $newUrl1 = BASE_URL . 'images/cred_info2.png';
                ?>
...
$(function() {
    $('#frm_cred_info').validate({
        ...
        submitHandler: function(form) {
            ...
            var base_url = $('#base_url').val();
            var global_valid = ValidateIPaddress('ip_address', true);
            if (global_valid == false) {
                error_msg_alert('Enter valid ip addresses!');
                return false;
            }
            if (new_password != '') {
                if (re_password == '') {
                    error_msg_alert("Retype New password");
                    return false;
                }
            }
                ...
                callback: function(data1) {
                    ...
                    if (data1 == "yes") {
                        ...
                        $.ajax({
                            ...
                            url: base_url +
                                'controller/app_settings/setting/app_cred_info_save.php',

                            data: {
                                sms_username: sms_username,
                                sms_password: sms_password,
                                app_email_id: app_email_id,
                                app_smtp_status: app_smtp_status,
                                app_smtp_host: app_smtp_host,
                                app_smtp_port: app_smtp_port,
                                app_smtp_password: app_smtp_password,
                                app_smtp_method: app_smtp_method,
