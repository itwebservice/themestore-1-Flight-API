...
$sq_cancel_amount = 0;
		while($row_payment = mysqli_fetch_assoc($sq_payment))
			if($row_payment['payment_amount']!=0){
			{
				...
				$url1 = BASE_URL."model/app_settings/print_html/receipt_html/receipt_body_html.php?payment_id_name=$payment_id_name&payment_id=$payment_id&receipt_date=$receipt_date&booking_id=$booking_id&customer_id=$customer_id&booking_name=$booking_name&travel_date=$travel_date&payment_amount=$payment_amount&transaction_id=$transaction_id&payment_date=$payment_date&bank_name=$bank_name&confirm_by=&receipt_type=$receipt_type&payment_mode=$payment_mode1&branch_status=$branch_status&outstanding=$outstanding&table_name=car_rental_payment&customer_field=booking_id&in_customer_id=$row_payment[booking_id]&status=$row_payment[status]";
