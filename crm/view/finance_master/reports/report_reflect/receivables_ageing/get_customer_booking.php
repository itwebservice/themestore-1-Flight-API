...
{	
	...
	while($row_package = mysqli_fetch_assoc($sq_pacakge))
	{
		...
		$cancel_est_count =mysqli_num_rows(mysqlQuery("select * from package_refund_traveler_estimate where booking_id='$row_package[booking_id]'"));
		...
		$cancel_est= ($cancel_est_count > 0) ? $cancel_est['cancel_amount'] : 0;
	...
	if($total_outstanding>'0'){
		...
		$temp_arr = array( "data" => array(
			...
			$customer_name,
			'Package Booking',
			"<button class='btn btn-info btn-sm' onclick='view_modal($booking_id_arr,$pending_amt_arr, $not_due_arr,$total_days_arr,$due_date_arr,$count)' id='".$count."' data-toggle='tooltip' title='Ageing Information'><i class='fa fa-eye'></i></button>",
	...
	while($row_package = mysqli_fetch_assoc($sq_pacakge))
	{
		// if($row_package['cancel_type'] == 0){
			...
			if($row_package['cancel_type'] == '1'){
				...
				$cancel_estimate_data = json_decode($row_package['cancel_estimate']);
				$cancel_estimate = (!isset($cancel_estimate_data)) ? 0 : $cancel_estimate_data[0]->ticket_total_cost;
				$pending_amt = (($booking_amt - floatval($cancel_estimate)) + $cancel_est) - $total_paid;
			}
			else{
				$pending_amt = $booking_amt - $total_paid;
			}
