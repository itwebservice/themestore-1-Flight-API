...
<form id="frm_save">
<div class="modal fade" id="jv_save_modal" role="dialog" aria-labelledby="myModalLabel" data-backdrop="static" data-keyboard="false">
...
$('#frm_save').validate({
    ...
    submitHandler:function(form){
      ...
      var base_url = $('#base_url').val();
        ...
        for(var i=0; i<rowCount; i++){
          ...
          if(row.cells[0].childNodes[0].checked){
              ...
              if(ledger_id==""){ error_msg_alert("Debit Ledger is required in row : "+(i+1)+"<br>");
                $('#btn_save').prop('disabled',false);return false; }
              if(amount==""){ error_msg_alert("Debit Amount is required in row : "+(i+1)+"<br>");
                $('#btn_save').prop('disabled',false); return false; }
        ...
        for(var i=0; i<rowCount; i++){
          ...
          if(row.cells[0].childNodes[0].checked){
              ...
              if(ledger_id==""){
                error_msg_alert("Credit Ledger is required in row : "+(i+1)+"<br>");
                $('#btn_save').prop('disabled',false); return false; }
              if(amount==""){
                error_msg_alert("Credit Amount is required in row : "+(i+1)+"<br>");
                $('#btn_save').prop('disabled',false); return false; }
      ...
      if(parseFloat(total_debit) != parseFloat(total_credit)){ 
        $('#btn_save').prop('disabled',false);
        error_msg_alert("Debit amount doesn't equal to Credit amount"); return false; }  
