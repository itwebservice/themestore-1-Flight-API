...
public function convert_number_to_words($number1,$currency_code='') {
  ...
  if($currency_code!=''){
    ...
    $number = isset($number_string[1]) ? $number_string[1] : $number_string[0];
  ...
  $result = implode('', $str);
  if($currency_code!=''){
    $currency_logo_d = mysqli_fetch_assoc(mysqlQuery("SELECT `currency_code` FROM `currency_name_master` WHERE id=" . $currency_code));
